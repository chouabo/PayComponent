 ...
 *
 */
public class WeiXinErrcodeUtil {
	...
	static {
		...
		errorCodeMap.put(40016, "菜单创建个数不能超过3个");
		errorCodeMap.put(40017, "菜单创建个数不能超过3个");
		...
		errorCodeMap.put(40053, "无效的操作信息,请检查文档");
		errorCodeMap.put(40054, "无效的url域名");
		...
		errorCodeMap
				.put(61454,
						"客服帐号名长度超过限制(仅允许10个英文字符，不包括@及@后的公众号的微信号)(invalidkf_acountlength)");
		errorCodeMap.put(61455,
				"客服帐号名包含非法字符(仅允许英文+数字)(illegalcharacterinkf_account)");
		errorCodeMap.put(61456, "客服帐号个数超过限制(10个客服账号)(kf_accountcountexceeded)");
		errorCodeMap.put(61457, "无效头像文件类型(invalidfiletype)");
		errorCodeMap.put(61458, "客户正在被其他客服接待(customer accepted by xxx@xxxx)");
		errorCodeMap.put(61459, "客服不在线(kf offline)");
	...
	public static String getErrorMsg(Integer errorCode) {
		if (errorCodeMap != null) {
			if (AppUtils.isNotBlank(errorCodeMap.get(errorCode))) {
